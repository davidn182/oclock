...
def calculate_apriori_dt(cd, correlations, plot=False,
                               **kwargs):
    '''
    ...
    sta2 = list(set([correlation.station2_code for correlation in correlations]))
    ...
    for correlation in correlations:
        ...
        if cd.get_station(correlation.station1_code).needs_correction ==True:
            correlation.apriori_dt1 = dt
            correlation.apriori_dt2 = 0
        elif cd.get_station(correlation.station2_code).needs_correction ==True:
...
def calculate_apriori_dt_corrected(cd, correlations, plot=False,
                               **kwargs):
    '''
    Calculates de apriori estimate of given several correlation files of the 
    same station pair, given that the correlation was perform in the same order
...
def check_input_correlation_list(correlation_list):
    ...
    '''
    if type(correlation_list) != list:
        raise Exception("correlations_list should be a list of Correlation objects")
    if len(correlation_list) == 0:
        print("No elements in correlations_list")
    else:
        for c in correlation_list:
            str_type = "<class 'main.Correlation'>"
            if str(type(c)) != str_type:
                raise Exception("list doesn't contain Correlation objects")

# def check_allstations_have_solutions(Clock_drift, stations):
#     '''
#     This method checks that all stations have a solution, if not then 
...
#     '''
#     for i in range(Clock_drift.iterations):
        
#         if Clock_drift.station[i].a
#     return
...
class Processing_parameters():
    ...
    def __init__(self, 
        ...
        self.snr_trh = float(snr_trh) # Signal-to-noise ratio threshold
        self.noise_st = noise_st # start of the noise window.
        self.dt_err = dt_err # Sampling interval needs to be multiple of this value.
    def __repr__(self):
...
class Clock_drift(object):
    ...
    def set_processing_parameters(self, list_of_processing_parameters):
        if type(list_of_processing_parameters) != list:
        ...
        for lst in list_of_processing_parameters:
            str_type = "<class 'main.Processing_parameters'>"
    ...
    def set_correlations(self, path2data_dir):
        ...
        for file in sorted(os.listdir(path2data_dir)):
            ...
            if (station1.needs_correction == False and
                station2.needs_correction == False):
                continue
            ...
            for processing_parameters in self.processing_parameters:
                correlation = Correlation(station1_code, station2_code, 
                                          ...
                                          delta, cpl_dist, processing_parameters)
    ...
    def calculate_tapp_4_allcorrelations(self, days_apart=50):
        ...
        for c in self.correlations:
            if (type(c.dt_ins_station1) == str or
                type(c.dt_ins_station2) == str):
        ...
        for station in self.stations:
            self.no_corr_per_avg_date(station, days_apart=days_apart, 
                                      plot=False)
    def stations_with_few_corrs(self, min_no_corr_periods, min_no_correlations4obs,
        ...
        If the station does not fit the criteria then it will not be included in 
        the inversion and all the correlations will be recounted after removing the
        correlations from this station.
        ...
        for params in self.processing_parameters:
            ...
            for correlation in correlations:
                ...
                if len(correlation.t_app) == 1:
                    ...
                    # print(correlation)
                    estimated_shift = self.calculate_estimated_shift(correlation)
    ...
    def no_corr_per_avg_date(self, station, days_apart=60, plot=True):
        ...
        '''
        obs_t = obspy.UTCDateTime
        avg_dates = sorted(dates_of_correlations_with_t_app(self, station.code))
        if len(avg_dates) == 0:
            station.no_corr_per_avg_date = {"No available correlations":0}
            return
        dates_unique = {avg_dates[0]:1}
        ...
        if plot:
            ...
            for date in avg_dates_witout_t_app[1:]:
                ...
                # its count.
                if dates_witout_t_app.get(date) != None:
            ...
            plt.figure(dpi=300)
            plt.bar(X-w, dates_unique.values(), label="Dates with t_app",width=0.25)
            plt.bar(X+w, dates_witout_t_app.values(), label="Dates without t_app",width=0.25)
    ...
    def plot_correlations_of_stationpair(self, station1_code, station2_code,
        ...
        for params in self.processing_parameters:
            ...
            for correlation in correlations:
                ...
                t1, data = trim_correlation_trace(tr, min_t, max_t, freqmin, freqmax)
                ax1.plot(t1, data, label=str(tr.stats.average_date)[:10], alpha=0.7)
    ...
    def plot_all_tappValues_and_solution_of_station(self, station_code):
        ...
        a_val = float(station.a[-1]) # Take the last a value that has been calculated.
        b_val = float(station.b[-1]) # Take the last a value that has been calculated.
        ref_t = self.reference_time
        plt.figure(dpi=300)
        all_dates = []
        for station in self.stations:
        ...
        plt.scatter([], [], color='royalblue', label='correlated with land station')
        ...
        for station in self.stations:
            ...
            for correlation in correlations:
                ...
                plt.scatter(iterations, t_apps, color='black', edgecolor='k', zorder=99,
                            alpha=0.5, marker='+')
                plt.plot(iterations, t_apps, color='red', alpha=1, linewidth=0.2)
    ...
    def plot_fluctuation_of_a_and_b(self):
        ...
        for station in self.stations:
            ...
            ax2.scatter(iterations, b_values, color='blue', edgecolor='k', zorder=99,
                    alpha=0.5, marker='+')
            ax2.plot(iterations, b_values, color='blue', alpha=1, linewidth=0.2)
        ...
        for station in self.stations:
            ...
            for correlation in self.correlations: 
                ...
                if self.iteration != 0:
                    ...
                    if station.included_in_inversion==False:
                        continue
                    if type(correlation.t_app[-1]) == str:
                        continue
    ...
    def plot_obs_and_pred_shifts_with_land_stations(self, station1_code,
        ...
        plt.plot(firstday_finalday, estimates, color="C3", alpha=0.9, zorder=-1)
    ...
    def plot_correlation(self, filepath, min_t=-40, max_t=40, freqmin=0.15, 
        ...
        ac_i0, ac_if = int(corr.acausal_from_index), int(corr.acausal_until_index)
        ...
        ax0.plot(t[c_i0:cif], data[c_i0:cif], label="Estimated causal and acausal wave", color='C3')
